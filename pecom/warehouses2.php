...
<head>
    ...
    <link rel="stylesheet" href="css/style_sclad.css" type="text/css">
        ...
        <div class="nav-main">
            <div class="container">
                ...
                <div class="nav-item "><a href="autotransport.php" class="nav-item-link"><span><i class="fas fa-car"></i>Транспорт</span></a></div>
                    <div class="nav-item active"><a href="warehouses.php" class="nav-item-link"><span><i class="fas fa-warehouse"></i>Склад</span></a></div>
...
?></span></a>
    ...
    <div class = "page">
        <div id="page" class="page-wrapper">
            <h1 class ="top">Склад</h1>
            <div class="layout-side">
                <div class="sidebar">
                        <a href="warehouses.php"><span class="main-span ">Предприятие</span></a>
                            <a href="warehouses2.php"><span class="main-span active">Партнеры</span></a>
                ...
                <div class=high-add>
                        <form method="POST">
                        <input class ="add" name="name" placeholder="Наименование">   
                        <input class ="add" name="add" placeholder="Адрес">
                        <input class ="add" name="add1" placeholder="Тип">
                        <input class ="add" name="add2" placeholder="Площадь">
...
$tip=$_POST['add1'];
$ploshad=$_POST['add2'];
$query="INSERT INTO sclad_partner SET name='$name',adres='$adres',tip='$tip', ploshad='$ploshad'";
mysqli_query($link, $query) or die(mysqli_error($link));
}
if(isset($_GET['del'])){
    $del=$_GET['del'];
    $query="DELETE FROM sclad_partner where id=$del";
    mysqli_query($link, $query) or die(mysqli_error($link));
    }
    $query="SELECT * FROM sclad_partner WHERE id > 0";
    $result=mysqli_query($link,$query) or die(mysqli_error($link));
    for($data=[];$row=mysqli_fetch_assoc($result);$data[]=$row);
    $result='';
    foreach($data as $elem){
        ...
        $result.='<td>'.$elem['adres'].'</td>';
        $result.='<td>'.$elem['tip'].'</td>';
        $result.='<td>'.$elem['ploshad'].'</td>';
