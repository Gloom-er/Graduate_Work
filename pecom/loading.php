            ...
            <div class="layout-side">
                <div class="sidebar">
                        <a href="trucking.php"><span class="main-span ">Автоперевозка</span></a>
                            ...
                            <a href="delivery.php"><span class="second-span ">Забор и доставка груза</span><br></a>
                            <a href="loading.php"><span class="second-span active">Погрузочно-разгрузочные работы</span><br></a>
                ...
                <div class=high-add>
                        ...
                        <input class ="add" name="add2" placeholder="Адрес отправления">
                        <input class ="add" name="add3" placeholder="Адрес доставки">
...
if(!empty($_POST)){ 
    ...
    $adres=$_POST['add2'];
    $adres2=$_POST['add3'];
$query="INSERT INTO loading SET name='$name',summa='$summa', adresot='$adres', adresdost='$adres2'";
mysqli_query($link, $query) or die(mysqli_error($link));
}
if(isset($_GET['del'])){
    $del=$_GET['del'];
    $query="DELETE FROM loading where id=$del";
    mysqli_query($link, $query) or die(mysqli_error($link));
    }
    $query="SELECT * FROM loading WHERE id > 0";
    $result=mysqli_query($link,$query) or die(mysqli_error($link));
    for($data=[];$row=mysqli_fetch_assoc($result);$data[]=$row);
    $result='';
    foreach($data as $elem){
