    ...
    <div class = "page">
        <div id="page" class="page-wrapper">
            ...
            <div class="layout-side">
                <div class="sidebar">
                        <a href="autotransport.php"><span class="main-span active">Автотранспорт</span></a>
                            ...
                            <a href="aviatransport.php"><span class="main-span">Авиатранспорт</span></a>
                    </div>
                ...
                <div class=high-add>
                        ...
                        <?php
                        $link=mysqli_connect('localhost', 'root', ''); // пароль, логин , хост нейм для подключения к базе
                        mysqli_select_db($link,'pec'); // название базы
                        $sql = "SELECT name FROM fuel";
                        $result = mysqli_query($link,$sql);
                        echo "<select name='topl' ><option >Выбор топлива</option>";
                        ...
                        $sql = "SELECT name FROM voditel";
                        $result = mysqli_query($link,$sql);
                        
                        echo "<select name='vod'><option>Выбор водителя</option>";
                        while ($row = mysqli_fetch_array($result)) {
                            echo "<option value='" . $row['name'] . "'>" . $row['name'] . "</option>";
                        }
                        echo "</select>";
                        ...
                        $sql = "SELECT name FROM pricep";
                        $result = mysqli_query($link,$sql);
                        
                        echo "<select name='pric'><option>Выбор прицепа</option>";
                        while ($row = mysqli_fetch_array($result)) {
                            echo "<option value='" . $row['name'] . "'>" . $row['name'] . "</option>";
                        }
                        echo "</select>";
...
$voditel=$_POST['vod'];
$pricep=$_POST['pric'];
$query="INSERT INTO auto SET name='$name',fuel='$fuel', voditel='$voditel',pricep='$pricep'";
mysqli_query($link, $query) or die(mysqli_error($link));
}
if(isset($_GET['del'])){
    $del=$_GET['del'];
    $query="DELETE FROM auto where id=$del";
    mysqli_query($link, $query) or die(mysqli_error($link));
    }
    $query="SELECT * FROM auto WHERE id > 0";
    $result=mysqli_query($link,$query) or die(mysqli_error($link));
    for($data=[];$row=mysqli_fetch_assoc($result);$data[]=$row);
    $result='';
    foreach($data as $elem){
        ...
        $result.='<td>'.$elem['fuel'].'</td>';
        $result.='<td>'.$elem['voditel'].'</td>';
        $result.='<td>'.$elem['pricep'].'</td>';
