            ...
            <div class="layout-side">
                <div class="sidebar">
                        <a href="zp.php"><span class="main-span ">Зарплата</span></a>
                            <a href="income.php"><span class="main-span active">Доходы</span></a>
...
if(!empty($_POST)){ 
    ...
    $source=$_POST['add1'];
    $data=$_POST['add2'];
$query="INSERT INTO dohod SET name='$name',source='$source', data='$data'";
mysqli_query($link, $query) or die(mysqli_error($link));
}
if(isset($_GET['del'])){
    $del=$_GET['del'];
    $query="DELETE FROM dohod where id=$del";
    mysqli_query($link, $query) or die(mysqli_error($link));
    }
    $query="SELECT * FROM dohod WHERE id > 0";
    $result=mysqli_query($link,$query) or die(mysqli_error($link));
    for($data=[];$row=mysqli_fetch_assoc($result);$data[]=$row);
    $result='';
    foreach($data as $elem){
