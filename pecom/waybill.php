...
<head>
    ...
    <link rel="stylesheet" href="css/style_doc.css" type="text/css">
        ...
        <div class="nav-main">
            <div class="container">
                ...
                <div class="nav-item "><a href="autotransport.php" class="nav-item-link"><span><i class="fas fa-car"></i>Транспорт</span></a></div>
                    ...
                    <div class="nav-item active"><a href="nomenclature.php" class="nav-item-link"><span><i class="fas fa-paste"></i>Документация</a></span></div>
...
?></span></a>
    ...
    <div class = "page">
        <div id="page" class="page-wrapper">
            <h1 class ="top">Документация</h1>
            <div class="layout-side">
                <div class="sidebar">
                <a href="nomenclature.php"><span class="main-span ">Номенклатура</span></a>
                            <a href="waybill.php"><span class="main-span active">Путевые листы</span></a>
                            <a href="order.php"><span class="main-span ">Приказы </span></a>
                            <a href="violation.php"><span class="main-span ">Нарушения </span></a>
                ...
                <div class=high-add>
                        ...
                        <input class ="add" name="add1" placeholder="Срок действия">
                        <input class ="add" name="add2" placeholder="Водитель">
...
if(!empty($_POST)){ 
    ...
    $term=$_POST['add1'];
    $voditel=$_POST['add2'];
    $transport=$_POST['add3'];
$query="INSERT INTO waybill SET name='$name',term='$term', voditel='$voditel',transport='$transport'";
mysqli_query($link, $query) or die(mysqli_error($link));
}
if(isset($_GET['del'])){
    $del=$_GET['del'];
    $query="DELETE FROM waybill where id=$del";
    mysqli_query($link, $query) or die(mysqli_error($link));
    }
    $query="SELECT * FROM waybill WHERE id > 0";
    $result=mysqli_query($link,$query) or die(mysqli_error($link));
    for($data=[];$row=mysqli_fetch_assoc($result);$data[]=$row);
    $result='';
    foreach($data as $elem){
        ...
        $result.='<td>'.$elem['term'].'</td>';
        $result.='<td>'.$elem['voditel'].'</td>';
