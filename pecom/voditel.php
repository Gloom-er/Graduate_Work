            ...
            <div class="layout-side">
                <div class="sidebar">
                    ...
                    <a href="autotransport.php"><span class="main-span ">Автотранспорт</span></a>
                            ...
                            <a href="voditel.php"><span class="second-span active">Водитель</span><br></a>
...
if(!empty($_POST)){ 
$name=$_POST['add'];
$query="INSERT INTO voditel SET name='$name'";
mysqli_query($link, $query) or die(mysqli_error($link));
}
if(isset($_GET['del'])){
    $del=$_GET['del'];
    $query="DELETE FROM voditel where id=$del";
    mysqli_query($link, $query) or die(mysqli_error($link));
    }
    $query="SELECT * FROM voditel WHERE id > 0";
    $result=mysqli_query($link,$query) or die(mysqli_error($link));
    for($data=[];$row=mysqli_fetch_assoc($result);$data[]=$row);
    $result='';
    foreach($data as $elem){
