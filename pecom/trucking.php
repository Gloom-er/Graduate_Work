    ...
    <div class = "page">
        <div id="page" class="page-wrapper">
            ...
            <div class="layout-side">
                <div class="sidebar">
                        <a href="trucking.php"><span class="main-span active">Автоперевозка</span></a>
...
if(!empty($_POST)){ 
    ...
    $adres=$_POST['add2'];
    $adres2=$_POST['add3'];
$query="INSERT INTO tracking SET name='$name',transport='$transport', adresot='$adres', adresdost='$adres2'";
mysqli_query($link, $query) or die(mysqli_error($link));
}
if(isset($_GET['del'])){
    $del=$_GET['del'];
    $query="DELETE FROM tracking where id=$del";
    mysqli_query($link, $query) or die(mysqli_error($link));
    }
    $query="SELECT * FROM tracking WHERE id > 0";
    $result=mysqli_query($link,$query) or die(mysqli_error($link));
    for($data=[];$row=mysqli_fetch_assoc($result);$data[]=$row);
    $result='';
    foreach($data as $elem){
