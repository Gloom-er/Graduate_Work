            ...
            <div class="layout-side">
                <div class="sidebar">
                        <a href="request_phis.php"><span class="main-span ">Физ лица</span></a>
                            <a href="request_ur.php"><span class="main-span active">Юр лица</span></a>
...
if(!empty($_POST)){ 
    ...
    $service=$_POST['add1'];
    $data=$_POST['add2'];
$query="INSERT INTO ur SET name='$name',service='$service', data='$data'";
mysqli_query($link, $query) or die(mysqli_error($link));
}
if(isset($_GET['del'])){
    $del=$_GET['del'];
    $query="DELETE FROM ur where id=$del";
    mysqli_query($link, $query) or die(mysqli_error($link));
    }
    $query="SELECT * FROM ur WHERE id > 0";
    $result=mysqli_query($link,$query) or die(mysqli_error($link));
    for($data=[];$row=mysqli_fetch_assoc($result);$data[]=$row);
    $result='';
    foreach($data as $elem){
